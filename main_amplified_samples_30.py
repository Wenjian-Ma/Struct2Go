...
def loading_item_data(func,name,data_source):
    item = []
    with open('./data_collect/amplify_samples/'+name+'_data_80.txt','r') as f:
        for line in f:
            line = line.strip('\n')
            item.append(line)
    ...
    if data_source=='pdb_amplifed' and name=='train':
        ...
        useless = []
        with open('./data_collect/amplify_samples/useless_from_7198_80.txt', 'r') as f:
            for line in f:
...
def train(args):
    ...
    for data_source in args.data:
        if data_source == 'pdb_no_amplifed':
            ...
            print('loading contact maps for pdb structures + alphafold structures...')
            dict_contact_alpha = loading_contact_map(data_source,args.threshold)
            print('loading seq for pdb structures + alphafold structures...')
    ...
    for func in args.func:
        for data_source in args.data:
            ...
            for e in range(args.Epoch):
                model = model.train()
                for batch_idx, batch in enumerate(tqdm(dataset_train,mininterval=0.5,desc='Training',leave=False,ncols=50)):
                ...
                with torch.no_grad():
                    for batch_idx,batch in enumerate(tqdm(dataset_valid,mininterval=0.5,desc='Validating',leave=False,ncols=50)):
                        data_prot = batch.to(device)
                ...
                #torch.save(model.state_dict(),'./data_collect/amplify_samples/model/'+func+'/' + str(data_source) + '_' + str(func) + '_' + args.net + '_' + str(args.hidden_dim) + '_' + args.pool + '_' + str(args.dropout) + '_' + str(args.threshold) +'_' +str(m_AUPR) +'.pkl')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--func', type=lambda s: [item for item in s.split(",")],
                        default=['cc'], help="list of func to predict.")#'mf','bp',  ,'bp','cc'
    parser.add_argument('--data', type=lambda s: [item for item in s.split(",")],
                        default=['pdb_amplifed'], help="data source.")#'pdb_no_amplifed','pdb_amplifed'
