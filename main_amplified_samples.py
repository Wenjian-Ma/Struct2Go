...
def loading_item_data(func,name,data_source):
    item = []
    with open('./data_collect/amplify_samples/'+name+'_data.txt','r') as f:
        for line in f:
            line = line.strip('\n')
            item.append(line)
    ...
    for func in args.func:
        for data_source in args.data:
            ...
            for e in range(args.Epoch):
                ...
                torch.save(model.state_dict(),'./data_collect/amplify_samples/model/' + str(data_source) + '_' + str(func) + '_' + args.net + '_' + str(args.hidden_dim) + '_' + args.pool + '_' + str(args.dropout) + '_' + str(args.threshold) +'_' +str(m_AUPR) +'.pkl')
