    ...
    for func in args.func:
        for data_source in args.data:
            ...
            if data_source == 'pdb':
                ...
                test_data = PFPDataset(train_data_X=test_item, train_contactmap_X=dict_contact_pdb,
                                       train_feature_matrix_X=dict_seq_pdb, train_data_Y=label,
                                       model_feature=model_feature, device=device)

            elif data_source == 'alpha':
                # train_data = PFPDataset(train_data_X=train_item, train_contactmap_X=dict_contact_alpha,
                #                         train_feature_matrix_X=dict_seq_alpha, train_data_Y=label,
                #                         model_feature=model_feature, device=device)
                #
                ...
                test_data = PFPDataset(train_data_X=test_item, train_contactmap_X=dict_contact_alpha,
                                       train_feature_matrix_X=dict_seq_alpha, train_data_Y=label,
                                       model_feature=model_feature, device=device)
            ...
            model = Model_Net(output_dim=label_num, net=args.net, hidden_dim=args.hidden_dim, pool=args.pool,
                              dropout=args.dropout).to(device)

            bceloss = nn.BCELoss()

            model_path = './data_collect/' + func + '/model/' + data_source + '_' + func + '_' + args.net + '_' + str(
                args.hidden_dim) + '_' + args.pool + '_' + str(args.dropout) + '_' + str(args.threshold) + '_7198.pkl'
            ...
            with torch.no_grad():
                ...
                print(data_source + '#################################')
                print('Test for ' + func + ':\t' + 'Test loss\t', loss_test.numpy(), '\tM-AUPR:\t',
                      perf['all']['M-aupr'], '\tm-AUPR:\t', perf['all']['m-aupr'], '\tF-max:\t', perf['all']['F-max'])
